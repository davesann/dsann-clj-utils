...
;; TODO - no need to refer to ui any more.
...
(defn update-data-paths [updaters old-ui-element new-ui-element lookup-type update-paths update-fn]
  (let [element-id (p-id/id new-ui-element)
...
(defn update-data-path-changed [updaters ui-element new-primary-path]
  (let [element-id (p-id/id ui-element)
...
(defn get-EXACT-update-fns [ui-state data-path]
  (if-let [element-map (get-in  ui-state [:updaters :data-paths :EXACT data-path])]
    (vals element-map)))
...
(defn get-data-paths [ui-state ui-element-id]
  (get-in ui-state [:updaters :ui-elemco-id/idelement-id :data-paths]))
